...
Lingua::EO::Orthography::JA - エスペラント文字の記法(正字法と代用表記)の変換器

=head1 バージョン

この文書はC<Lingua::EO::Orthography>のバージョンC<0.00>について記述しています。
...
L<Lingua::EO::Orthography::JA|Lingua::EO::Orthography::JA> (この文書)
...
=head1 概要
    ...
    $source = q(C^i-momente, la songha h'orajxo )
            . q(^sprucigas aplauwdon.);         # 変換元: 代用表記
    $target = $converter->convert($source);     # 変換先: UTF-8 正字法
    # この文は「幻想的な合唱曲への拍手がこの瞬間に噴出した。」という意味です
    ...
    $source = "\x{108}i-momente, la son\x{11D}a \x{125}ora\x{135}o "
            . "\x{153}prucigas apla\x{16D}don"; # 変換元: UTF-8 正字法
    $target = $converter->convert($source);     # 変換先: X方式

=head1 解説

エスペラントのアルファベットのうち6文字はASCIIに存在しません。
歴史的に、つまり活版印刷やタイプライターの時代から、それらの文字はしばしばL<代用表記|/記法>されて来ました。
現在ではユニコード(Unicode)の普及によって正字法で表記することも珍しくありません。しかしキーボードからの入力が難しい環境もまだ多いでしょうし、代用表記で記述された昔の文書を取り扱うこともあるでしょう。

このオブジェクト指向モジュールは、それらの記法を変換する機能を提供します。

=head2 警告

B<このモジュールはベータ版リリースの段階であり、APIは将来的に変更され得ます。
あなたのフィードバックを歓迎します。>

=head2 記法

以下の記法名はL<new()|/new>やL<add_sources()|/add_sources>などの引数に使えます。
...
これはL<Encode|Encode>モジュールの定石と同じです。
...
エスペラントの創始者であるザメンホフ博士が『I<エスペラントの基礎>』(I<Fundamento de Esperanto>)で提案した代用表記であり、I<ザメンホフ方式>(I<Zamenhof system>, I<Zamenhofa sistemo>)と呼ばれています。
このため「第二の正字法」とも称されていますが、今日ではあまり使われていません。

この方式には、'flughaveno'(空港: flug/haven/o)などの単語で、語根を跨ぐ文字の並びが代用表記されているように見えるという問題があります。
...
I<H方式>(I<H-system>, I<H-sistemo>)と呼ばれています。
...
I<X方式>(I<X-system>, I<X-sistemo>)と呼ばれています。
...
I<キャレット方式>(I<caret system>, I<**** sistemo>)と呼ばれています。

キャレット(caret)がサーカムフレックス(circumflex)と同じ形であることから、代用表記の一つとして使われることがあります。
...
このモジュールと同等の機能を提供するモジュールとして、C<Lingua::EO::Orthography|Lingua::EO::Orthography>がCPANにあります。
以下の表でそれらを比較します。

 観点                       ::Supersignoj   ::Orthography               備考
 -------------------------- --------------- --------------------------- ----
 バージョン                 0.02            0.00
 @linesを変換できる         はい            いいえ                      *1
 アクセッサーがある         はい            はい(有用なメソッド付き)    *2
 記法をカスタマイズできる   'u'のみ可能     いいえ(検討中)              *3
 'flughaveno'を取り扱える   いいえ          いいえ(検討中)              *4
 APIの言語                  eo:エスペラント en:英語
 多対1の変換ができる        いいえ          はい                        *5
 複数の記法を取り扱える     いいえ          はい                        *5
 速度                       満足な速さ      約4倍速い                   *6
 直接の依存                 3 (コアは2)     9 (コアは4)                 *7
 全体の依存                 3 (コアは2)     22 (コアは12)               *7
 テストケース数             3               ****
...
さらに、L<Lingua::EO::Orthography|Lingua::EO::Orthography>は複数の記法による文字を一度に変換することが出来ます。

このディストリビューションに同梱されているF</examples/benchmark.pl>を調べてください。
...
何かバグを発見されたら、機能のご要望がありましたら、または改善のためのご意見がありましたら、メール(C<bug-lingua-eo-orthography at rt.cpan.org>宛)で報告してください。
