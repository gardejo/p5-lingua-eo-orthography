...
=head1 概要
    ...
    $converted = $converter->convert($original);    # 変換先: UTF-8 正字法
    ...
    $converted = $converter->convert($original);    # 変換先: X方式
...
この方式には、'flughaveno'(空港)などの単語で、語根('flug/haven/o')を跨ぐ文字の並びが代用表記されているように見えるという問題があります。
...
文字列の大文字・小文字は区別されます。
つまり、L<postfix_x記法|/postfix_x>であってもC<cX>は代用表記と見なさず、変換しません。
...
ユーザー辞書によってC<flug/haven/o>などの単語をL<postfix_h記法|/postfix_h>で正確に取り扱うこと

=item *

ユーザー辞書によってC<ankaŭ>などの単語をL<zamenhof記法|/zamenhof>で正確に取り扱うこと
